...
{
public:
  explicit CursorIndicator(ControllerEmu::Cursor& cursor, int wiimote_port = -1)
      : m_cursor_group(cursor), m_wiimote_port(wiimote_port)
  {
  }
...
public:
  explicit AccelerometerMappingIndicator(ControllerEmu::IMUAccelerometer& accel,
      ...
      : m_accel_group(accel), m_wiimote_port(wiimote_port),
        m_prefer_left_hand(prefer_left_hand)
  {
  }
...
private:
  ...
  int m_wiimote_port = -1;
  bool m_prefer_left_hand = false;
};

class GyroMappingIndicator : public SquareIndicator
{
public:
  explicit GyroMappingIndicator(ControllerEmu::IMUGyroscope& gyro, int wiimote_port = -1)
      : m_gyro_group(gyro), m_wiimote_port(wiimote_port)
  {
  }
