...
namespace
{
WiimoteSource GetSourceFromCombo(const QComboBox* combo)
{
  const QVariant value = combo->currentData();
  if (!value.isValid())
    return WiimoteSource::None;

  return static_cast<WiimoteSource>(value.toInt());
}

bool IsConfigurableWiimoteSource(WiimoteSource source)
{
...
void WiimoteControllersWidget::CreateLayout()
{
  ...
  for (size_t i = 0; i < m_wiimote_groups.size(); i++)
  {
    ...
    wm_box->addItem(tr("None"), static_cast<int>(WiimoteSource::None));
    wm_box->addItem(tr("Emulated Wii Remote"), static_cast<int>(WiimoteSource::Emulated));
    wm_box->addItem(tr("OpenXR Wii Remote"), static_cast<int>(WiimoteSource::OpenXR));
    wm_box->addItem(tr("Real Wii Remote"), static_cast<int>(WiimoteSource::Real));
...
void WiimoteControllersWidget::OnWiimoteConfigure(size_t index)
{
  ...
  switch (GetSourceFromCombo(m_wiimote_boxes[index]))
  {
...
void WiimoteControllersWidget::LoadSettings(Core::State state)
{
  for (size_t i = 0; i < m_wiimote_groups.size(); i++)
  {
    const WiimoteSource source = Config::Get(Config::GetInfoForWiimoteSource(int(i)));
    int source_index = m_wiimote_boxes[i]->findData(static_cast<int>(source));
    if (source_index < 0)
      source_index = m_wiimote_boxes[i]->findData(static_cast<int>(WiimoteSource::None));
    if (source_index < 0)
      source_index = 0;

    SignalBlocking(m_wiimote_boxes[i])->setCurrentIndex(source_index);
  }
  ...
  for (size_t i = 0; i < m_wiimote_groups.size(); i++)
  {
    ...
    const WiimoteSource source = GetSourceFromCombo(m_wiimote_boxes[i]);
    m_wiimote_buttons[i]->setEnabled(enable_emu_bt && IsConfigurableWiimoteSource(source) &&
...
{
  {
    ...
    {
      const WiimoteSource source = GetSourceFromCombo(m_wiimote_boxes[i]);
      Config::SetBaseOrCurrent(Config::GetInfoForWiimoteSource(int(i)), source);
    }
  }
