...
bool PostProcessing::CanBlitFromTextureLayered() const
{
  ...
  // Vulkan writes both layers in one pass via VK_KHR_multiview (gl_ViewIndex).
  if (g_backend_info.api_type == APIType::Vulkan)
    return g_backend_info.bSupportsMultiview && m_multiview_vertex_shader != nullptr;

  // D3D writes both layers in one pass via the passthrough geometry shader
  // (SV_RenderTargetArrayIndex + slice in v_tex0.z), same as stereo EFB copies.
...
bool PostProcessing::CompilePipeline()
{
  ...
  // Single-pass layered blit pipelines for OpenXR layered swapchains: Vulkan selects the
  // layer with VK_KHR_multiview, D3D with the passthrough geometry shader (which also
  // rewrites v_tex0.z, so the pixel shader samples the matching source layer).
