...
import org.dolphinemu.dolphinemu.ui.launcher.AnimalCrossingVrLauncherActivity
   ...
   */
    ...
    fun finishEmulationActivity() {
        ...
        if (emulationActivity == null) {
            ...
            emulationActivity.runOnUiThread {
                // Quest VR launches start EmulationActivity in a new task and finish the launcher,
                // so the back stack is empty. Return to the dedicated Animal Crossing VR frontend
                // rather than exposing Dolphin's library screen.
                if (emulationActivity.isTaskRoot) {
                    val intent = Intent(emulationActivity, AnimalCrossingVrLauncherActivity::class.java)
