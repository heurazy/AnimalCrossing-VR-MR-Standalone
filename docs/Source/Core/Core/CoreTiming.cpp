...
static float GetEffectiveTotalOverclockFactor()
{
  const float cpu_factor =
      Config::Get(Config::MAIN_OVERCLOCK_ENABLE) ? Config::Get(Config::MAIN_OVERCLOCK) : 1.0f;
  return cpu_factor * GetEffectiveVIOverclockFactor();
}
...
{
  m_config_oc_factor = GetEffectiveTotalOverclockFactor();
