...
bool VideoBackend::Initialize(const WindowSystemInfo& wsi)
{
  ...
  if (g_ActiveConfig.stereo_mode == StereoMode::OpenXR)
  {
    ...
    {
      DX11::g_openxr_d3d = std::move(openxr);
    }
  }
#endif
...
{
#ifdef ENABLE_VR
  // Shut down VR before the D3D device is destroyed.
  // g_openxr_d3d first (releases DXTexture/DXFramebuffer wrappers),
  // then g_openxr (calls xrDestroySwapchain/Session/Instance — releases runtime D3D11 refs).
  // Both must happen before D3D::Destroy() to avoid "Unreleased references" warnings.
  DX11::g_openxr_d3d.reset();
  VR::g_openxr.reset();
#endif
  ShutdownShared();
  D3D::Destroy();
