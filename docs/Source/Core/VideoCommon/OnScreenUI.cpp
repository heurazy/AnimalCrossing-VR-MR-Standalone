      ...
      else
      {
        ...
        for (size_t i = 0; i < status.texture_filters.size() && i < max_hashes_to_show; i++)
        {
          if (!texture_line.empty())
            texture_line += ", ";
          texture_line += fmt::format("{:016x}", status.texture_filters[i]);
        }
        ...
        ImGui::TextUnformatted(fmt::format("Texture Hashes: {}", texture_line).c_str());
      }

      if (status.group_hunt_enabled)
      {
        const char* seed_type_text =
            status.group_hunt_seed_type == ShaderHunter::ShaderType::Vertex   ? "VS" :
            status.group_hunt_seed_type == ShaderHunter::ShaderType::Geometry ? "GS" :
                                                                                "PS";
        ImGui::Separator();
        if (status.group_hunt_seed_valid)
        {
          ImGui::Text("Group Hunt Seed: %s 0x%016llX", seed_type_text,
                      static_cast<unsigned long long>(status.group_hunt_seed_hash));
          if (status.group_hunt_seed_signature.use_projection ||
          ...
          {
            ImGui::Text("Group Match: %d / %d",
                        status.group_hunt_total_matches > 0 ? status.group_hunt_selected_match + 1 :
                                                              0,
                        status.group_hunt_total_matches);
            if (status.group_hunt_draw_index >= 0)
            {
              ImGui::Text("Group Draw: #%d  0x%016llX", status.group_hunt_draw_index + 1,
                          static_cast<unsigned long long>(status.group_hunt_current_hash));
            }
            else
            {
              ImGui::TextUnformatted("Group Draw: (not seen this frame)");
            }
          }
          else
          {
            ImGui::TextUnformatted("Group Match: select at least one signature group");
          }
        }
        else
        {
          ImGui::TextUnformatted("Group Hunt: awaiting seed draw");
        }
      }
    }
    ImGui::End();
