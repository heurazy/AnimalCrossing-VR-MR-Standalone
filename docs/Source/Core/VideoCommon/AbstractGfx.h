...
public:
  ...
  // Override just the pixel shader after SetPipeline (used for shader hunter pink highlight).
  // Backends that support this should also invalidate cached pipeline state.
  virtual void SetForcePixelShader(const AbstractShader* shader) {}
