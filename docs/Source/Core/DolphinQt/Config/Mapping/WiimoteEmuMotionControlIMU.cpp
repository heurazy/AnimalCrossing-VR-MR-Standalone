...
{
  const WiimoteSource source = Config::Get(Config::GetInfoForWiimoteSource(GetPort()));

  auto* groups_layout = new QHBoxLayout();
  groups_layout->addWidget(
      CreateGroupBox(Wiimote::GetWiimoteGroup(GetPort(),
                                              source == WiimoteSource::OpenXR ?
  ...
  if (source != WiimoteSource::OpenXR)
  {
    groups_layout->addWidget(CreateGroupBox(
        Wiimote::GetWiimoteGroup(GetPort(), WiimoteEmu::WiimoteGroup::IRPassthrough)));
    groups_layout->addWidget(CreateGroupBox(
