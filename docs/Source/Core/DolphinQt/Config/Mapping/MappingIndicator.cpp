...
std::optional<Wiimote::OpenXRWiiRemoteState>
GetOpenXRWiiRemoteStateForGroup(int wiimote_port, const std::string& group_name,
                                bool prefer_left_hand = false)
{
  if (prefer_left_hand)
    return Wiimote::GetOpenXRHandState(true);

  if (wiimote_port < WIIMOTE_CHAN_0 || wiimote_port >= MAX_WIIMOTES)
    return std::nullopt;

  if (group_name != WiimoteEmu::Wiimote::IR_GROUP &&
      group_name != WiimoteEmu::Wiimote::ACCELEROMETER_GROUP &&
      group_name != WiimoteEmu::Wiimote::GYROSCOPE_GROUP)
  {
    return std::nullopt;
  }

  return Wiimote::GetOpenXRWiiRemoteState(static_cast<unsigned int>(wiimote_port));
}
...
{
  if (const auto openxr_state =
          GetOpenXRWiiRemoteStateForGroup(m_wiimote_port, m_cursor_group.name))
  {
    const auto adj_coord =
        openxr_state->ir_visible
            ? std::make_optional(ControllerEmu::ReshapableInput::ReshapeData{openxr_state->ir_x,
                                                                              openxr_state->ir_y})
            : std::nullopt;
    DrawReshapableInput(m_cursor_group, adj_coord);
    return;
  }
...
{
  auto accel_state = m_accel_group.GetState();
  if (const auto openxr_state =
          GetOpenXRWiiRemoteStateForGroup(m_wiimote_port, m_accel_group.name, m_prefer_left_hand))
  {
    accel_state = Common::Vec3{openxr_state->acceleration[0], openxr_state->acceleration[1],
                               openxr_state->acceleration[2]};
  }
...
{
  auto gyro_state = m_gyro_group.GetState();
  if (const auto openxr_state = GetOpenXRWiiRemoteStateForGroup(m_wiimote_port, m_gyro_group.name))
  {
    gyro_state = Common::Vec3{openxr_state->angular_velocity[0], openxr_state->angular_velocity[1],
                              openxr_state->angular_velocity[2]};
  }
...
{
  auto gyro_state = m_gyro_group.GetState();
  auto raw_gyro_state = m_gyro_group.GetRawState();
  if (const auto openxr_state = GetOpenXRWiiRemoteStateForGroup(m_wiimote_port, m_gyro_group.name))
  {
    const Common::Vec3 openxr_angular_velocity{
        openxr_state->angular_velocity[0], openxr_state->angular_velocity[1],
        openxr_state->angular_velocity[2]};
    gyro_state = openxr_angular_velocity;
    raw_gyro_state = openxr_angular_velocity;
  }
  const auto angular_velocity = gyro_state.value_or(Common::Vec3{});
