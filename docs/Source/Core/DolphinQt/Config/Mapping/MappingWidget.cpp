...
QGroupBox* MappingWidget::CreateGroupBox(const QString& name, ControllerEmu::ControlGroup* group)
{
  ...
  const bool is_openxr_wiimote_source =
      GetPort() >= WIIMOTE_CHAN_0 && GetPort() < MAX_WIIMOTES &&
      Config::Get(Config::GetInfoForWiimoteSource(GetPort())) == WiimoteSource::OpenXR;
  const bool is_openxr_wiimote_point_group =
      is_openxr_wiimote_source &&
      group == Wiimote::GetWiimoteGroup(GetPort(), WiimoteEmu::WiimoteGroup::Point);
  const bool is_openxr_wiimote_imu_accel_group =
      is_openxr_wiimote_source &&
      group == Wiimote::GetWiimoteGroup(GetPort(), WiimoteEmu::WiimoteGroup::IMUAccelerometer);
  ...
  case ControllerEmu::GroupType::Cursor:
    indicator = new CursorIndicator(*static_cast<ControllerEmu::Cursor*>(group), GetPort());
    break;
  ...
  {
    int indicator_port = -1;
    bool prefer_left_hand = false;
    if (GetPort() >= WIIMOTE_CHAN_0 && GetPort() < MAX_WIIMOTES)
    {
      auto* const wiimote_group = Wiimote::GetWiimoteGroup(
          GetPort(), WiimoteEmu::WiimoteGroup::IMUAccelerometer);
      if (group == wiimote_group)
      {
        indicator_port = GetPort();
      }
      else
      {
        auto* const nunchuk_group =
            Wiimote::GetNunchukGroup(GetPort(), WiimoteEmu::NunchukGroup::IMUAccelerometer);
        if (group == nunchuk_group)
        {
          indicator_port = GetPort();
          prefer_left_hand = true;
        }
      }
    }

    indicator = new AccelerometerMappingIndicator(
        *static_cast<ControllerEmu::IMUAccelerometer*>(group), indicator_port, prefer_left_hand);
    break;
  }
  ...
  if (indicator != nullptr)
  {
    const bool need_calibration = !is_openxr_wiimote_point_group &&
                                  (group->type == ControllerEmu::GroupType::Cursor ||
  ...
  if (group->type == ControllerEmu::GroupType::Cursor && !is_openxr_wiimote_point_group)
  {
